...
void ia_primitive(struct Game *Game)
{
    ...
    if (nb_coups_possibles>Game->coups_possibles) Game->coups_possibles=nb_coups_possibles;
    return;
}
...
int main()
{
    ...
    for (int i=0;i<=100;i++)
{
    struct Game Game={StartBoard,0,0,J1,0,0};
    Game.Board[3][3]=J2;
    Game.Board[3][4]=J1;
    Game.Board[4][3]=J1;
    Game.Board[4][4]=J2;
    ...
    game_ended(Game);
    sleep(1);
    if (Game.coups_possibles>coups_possibles) coups_possibles=Game.coups_possibles;
}
    printf("\n\n\n%d\n",coups_possibles);
    return 0;
}